...
def Build_Character_Encoder():
    ...
    Character_Encoder.update(zip(CharList1,CodeList1))
    Character_Encoder.update(zip(CharList2,CodeList2))
...
def Build_Character_Decoder():
    ...
    CharList2 = list('0123456789')
    Character_Decoder = {}
    Character_Decoder.update(zip(CodeList1,CharList1))
    Character_Decoder.update(zip(CodeList2,CharList2))
    Character_Decoder.update({'.-.-.-':'#'})
...
def Word_Encoder(Word, Character_Encoder=None):
    ...
    Temp = []
    for j in Word:
        if j in Character_Encoder.keys():
            Temp.append(Character_Encoder[j])
        elif j not in Character_Encoder.keys():
            Temp.append('.-.-.-')
        
    Word_in_Morse_Code = Word_in_Morse_Code.join(Temp)
...
def Word_Decoder(Word_in_Morse_Code, Character_Decoder=None):
    ...
    # add your code from here
    Word_in_Morse_Code = Word_in_Morse_Code.join()
    Word = ''
    for j in Word_in_Morse_Code:
        if j in Character_Decoder.keys():
            Word += Character_Decoder[j]
        else:
            Word += '#'
...
def Message_Encoder(Message, Character_Encoder=None):
    ...
    for j in Message:
        for letter in j:
            if letter in Character_Encoder.keys():
                Message_in_Morse_Code += Character_Encoder[letter]
                Message_in_Morse_Code += ' '
            elif letter not in Character_Encoder.keys():
                Message_in_Morse_Code += '.-.-.-'
                Message_in_Morse_Code += ' '
        Comma+=1
        if Comma < len(Message):
            Message_in_Morse_Code += ','
...
def Message_Decoder(Message_in_Morse_Code, Character_Decoder=None):
    ...
    for j in Message_in_Morse_Code:
        temp = j.split()
        for character in temp:
            if character in Character_Decoder.keys():
                Message+= Character_Decoder[character]
            else:
                Message+='#'
        if space < len(Message_in_Morse_Code)-1:
            Message+=' '
            space+=1
    return Message
