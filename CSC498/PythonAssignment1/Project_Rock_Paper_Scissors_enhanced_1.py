...
def SmartAI(GameRecord):
    '''
    There is probably a better way to do this but this was the "easiest"
    '''
    Rock = ['Rock','rock','r','R']
    Paper = ['Paper','paper','p','P']
    Scissors = ['Scissors','scissors','s','S']
    TRounds = len(GameRecord[2])
    Rfreq = 0
    Pfreq = 0
    Sfreq = 0

    for i in GameRecord[1]:
        if i in Rock:
            Rfreq+=1
        elif i in Paper:
            Pfreq+=1
    ...
    draw = np.random.choice(['Rock','Paper','Scissors'],1,p=[Sfreq,Rfreq,Pfreq])[0]
    return draw
...
def PlayGame():
    ...
    while True:
        ...
        if len(GameRecord[2]) > 9:
            Computer = SmartAI(GameRecord)
